...
/* === MetaX detection-core stub ===
   The original parse_communicator reverse-engineers NCCL's private `ncclComm`
   memory layout (see HackedComm below) to read nRanks/rank/nChannels/ring/tree.
   MCCL's internal layout differs and is not needed for the fail-slow detection
   core (which only needs rank/nRanks to group records per rank/comm).  We use
   the *public* MCCL API instead: mcclCommUserRank + mcclCommCount, resolved via
   dlsym from libmccl at first use.  Ring/tree topology is left empty; that only
   feeds the mitigation-side topology cache, not the detector. */
#include <dlfcn.h>

typedef ncclResult_t (*mcclCommCountFn)(const ncclComm_t, int*);
typedef ncclResult_t (*mcclCommUserRankFn)(const ncclComm_t, int*);

static mcclCommCountFn    p_comm_count    = nullptr;
static mcclCommUserRankFn p_comm_userrank = nullptr;

static void ensure_mccl_query_fns()
{
    if (p_comm_count && p_comm_userrank)
        return;
    // RTLD_DEFAULT: libmccl is already loaded (torch depends on it); grab the
    // real symbols from the global scope so we don't recurse into any hook.
    const char* path = get_nccl_path("MCCL_PATH");
    void* h = path ? dlopen(path, RTLD_LAZY | RTLD_NOLOAD) : nullptr;
    if (!h)
        h = dlopen("/opt/maca/lib/libmccl.so", RTLD_LAZY | RTLD_NOLOAD);
    if (!h)
        h = dlopen("/opt/maca/lib/libmccl.so", RTLD_LAZY);
    if (h) {
        if (!p_comm_count)
            p_comm_count = reinterpret_cast<mcclCommCountFn>(dlsym(h, "mcclCommCount"));
        if (!p_comm_userrank)
            p_comm_userrank = reinterpret_cast<mcclCommUserRankFn>(dlsym(h, "mcclCommUserRank"));
    }
}

void parse_communicator(ncclComm_t hidden_comm, Communicator* parsed_comm)
{
    parsed_comm->comm_addr = reinterpret_cast<uint64_t>(hidden_comm);

    int nranks = 0, group_rank = COMM_RANK_UNDEFINED;
    ensure_mccl_query_fns();
    if (p_comm_count && hidden_comm)
        p_comm_count(hidden_comm, &nranks);
    if (p_comm_userrank && hidden_comm)
        p_comm_userrank(hidden_comm, &group_rank);

    parsed_comm->num_devices = nranks;
    parsed_comm->group_rank  = group_rank;

    // For torchrun/ompi, RANK and LOCAL_RANK are set in environment variables.
    parsed_comm->local_rank  = get_local_rank(DistEngine::auto_find);
    parsed_comm->global_rank = get_rank(DistEngine::auto_find);

    // Topology (ring/tree/#channels) is not decoded in detection-only mode.
    parsed_comm->num_channels = 0;
}

#else  // original NVIDIA-NCCL reverse-engineered path
...
#endif  // GREYHOUND_DETECTION_ONLY
