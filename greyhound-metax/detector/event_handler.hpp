...
#else
/* Detection-core build: the control plane (redis-driven pause/validate signals
   and p2p/gemm micro-benchmarks) is disabled.  EventHandler becomes a no-op so
   that the monitor path (hook -> shm ring buffer) links without cpp_redis. */
class EventHandler
{
    ncclComm_t world_comm;
    Communicator parsed_comm;
public:
    EventHandler(std::string, int, ncclSendFuncPtr, ncclRecvFuncPtr) : world_comm(nullptr) {}
    ~EventHandler() {}
    bool has_world_comm() const { return world_comm != nullptr; }
    void set_world_comm(ncclComm_t comm) { world_comm = comm; }
    void fetech_and_exec_task() {}
    void handle_control_signal(cudaStream_t, ControlState*) {}
};
#endif
