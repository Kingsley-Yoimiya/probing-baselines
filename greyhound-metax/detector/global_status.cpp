...
void GlobalStatus::initialize(const char* nccl_path_)
{
    ...
    // First, find & load the MCCL/NCCL dynamic lib.
    // On MetaX the collective library is libmccl.so (exports mccl* symbols);
    // the probe dlsym's the real mccl* functions from here to forward calls.
    if (nccl_path_ == nullptr)
#ifdef GREYHOUND_DETECTION_ONLY
        this->nccl_lib_path = std::string("/opt/maca/lib/libmccl.so");
#else
...
#ifdef GREYHOUND_DETECTION_ONLY
    // MetaX: real collective symbols are mccl*; the no-op EventHandler ignores
    // these pointers anyway, but resolve them correctly for consistency.
    this->event_handler = std::shared_ptr<EventHandler>(
        new EventHandler(get_master_addr(), get_redis_port(),
            reinterpret_cast<ncclSendFuncPtr>(get_function_ptr("mcclSend")),
            reinterpret_cast<ncclRecvFuncPtr>(get_function_ptr("mcclRecv")))
    );
#else
    this->event_handler = std::shared_ptr<EventHandler>(
        new EventHandler(get_master_addr(), get_redis_port(),
