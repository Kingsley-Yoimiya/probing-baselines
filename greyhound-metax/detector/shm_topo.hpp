...
#else
/* Detection-core build: topo cache is redis-backed, and comm-topology reverse
   engineering is stubbed out. We keep a header-only no-op so that global_status
   can still hold a shared_ptr<NcclTopoConnection> without linking cpp_redis. */
class NcclTopoConnection
{
public:
    NcclTopoConnection(int) {}
    ~NcclTopoConnection() {}
    std::shared_ptr<Communicator> add_comm(Communicator&) { return nullptr; }
    std::shared_ptr<Communicator> find(uint64_t) { return nullptr; }
    std::vector<std::shared_ptr<Communicator> > find_linked_ranks(const std::shared_ptr<Communicator>) {
        return {};
    }
    uint64_t num_comms() { return 0; }
};
#endif
